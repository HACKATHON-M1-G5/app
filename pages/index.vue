  ...
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="hero bg-base-100 shadow-xl rounded-box mb-8">
      <div class="hero-content text-center py-8 md:py-12">
        <div class="max-w-2xl px-2">
          ...
          <div v-if="!user" class="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
            <NuxtLink to="/register" class="btn btn-primary btn-md sm:btn-lg">
              Commencer maintenant
            </NuxtLink>
            <NuxtLink to="/login" class="btn btn-outline btn-md sm:btn-lg"> Se connecter </NuxtLink>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="user && userData">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <NuxtLink
          to="/groups/create"
          class="card bg-primary text-primary-content shadow-xl hover:shadow-2xl transition-shadow"
        >
        ...
        <NuxtLink
          to="/groups"
          class="card bg-secondary text-secondary-content shadow-xl hover:shadow-2xl transition-shadow"
        >
          <div class="card-body items-center text-center">
            <div class="text-4xl md:text-5xl mb-2">🏆</div>
        ...
        <NuxtLink
          to="/public-bets"
          class="card bg-accent text-accent-content shadow-xl hover:shadow-2xl transition-shadow"
        >
          <div class="card-body items-center text-center">
            <div class="text-4xl md:text-5xl mb-2">🌍</div>
      ...
      <section class="mb-8">
        ...
        >
          <h2 class="text-xl md:text-2xl font-bold">Paris Publics Récents</h2>
          <NuxtLink
            to="/public-bets"
            class="btn btn-ghost btn-xs sm:btn-sm self-start sm:self-auto"
          >
            Voir tout →
          </NuxtLink>
        </div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <PronoCard v-for="prono in publicPronos.slice(0, 3)" :key="prono.id" :prono="prono" />
        </div>
      </section>
      ...
      <section v-if="myTeams.length > 0">
        <div
          class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 mb-4"
        >
          <h2 class="text-xl md:text-2xl font-bold">Mes Groupes</h2>
          <NuxtLink to="/groups" class="btn btn-ghost btn-xs sm:btn-sm self-start sm:self-auto">
            Voir tout →
          </NuxtLink>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <GroupCard v-for="team in myTeams.slice(0, 3)" :key="team.id" :team="team" />
        </div>
      </section>
              ...
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                <div>
                  ...
                  <p class="text-sm text-success">
                    Gain potentiel :
                    {{ Math.floor(userBet.amount * (userBet.bet?.odds || 1)) }} tokens
                  </p>
                </div>

                <NuxtLink
                  :to="`/pronos/${userBet.bet?.prono?.id}`"
                  class="btn btn-primary btn-xs sm:btn-sm self-start"
                >
                  Voir
                </NuxtLink>
              </div>
